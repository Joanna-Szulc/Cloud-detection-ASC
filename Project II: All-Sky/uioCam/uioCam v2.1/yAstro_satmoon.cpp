	...
	MoonData md[S_NMOONS])		/* return info */
{
	...
	*sizep = degrad(sop->s_size / 3600.0);
	...
	md[1].mag = (float)13; md[2].mag = (float)11.8; md[3].mag = (float)10.3; md[4].mag = (float)10.2;
	md[5].mag = (float)9.8; md[6].mag = (float)8.4; md[7].mag = (float)14.3; md[8].mag = (float)11.2;
...
#define	SATRAU	.0004014253	/* saturn radius, AU */
	...
	/* open */
	(void)sprintf_s(buf, 1000, "%s/%s", dir, fn);
	errVal = fopen_s(&fp, buf, "r");
	if (!fp) {
	...
	for (i = 1; i < S_NMOONS; i++) {
		md[i].x = (float)(x[i - 1] / SATRAU);	/* we want sat radii +E */
		md[i].y = (float)(-y[i - 1] / SATRAU);	/* we want sat radii +S */
		md[i].z = (float)(-z[i - 1] / SATRAU);	/* we want sat radii +front */
	}
	...
	MoonData md[S_NMOONS])	/* fill md[1..NM-1].x/y/z for each moon */
{
	...
	*/
	...
	for (I = 1; I < S_NMOONS; I++) {
		md[I].x = (float)X[I];	/* we want +E */
		md[I].y = (float)-Y[I];	/* we want +S */
		md[I].z = (float)-Z[I];	/* we want +front */
	}
}
	...
	MoonData md[S_NMOONS])	/* fill in RA and Dec */
{
	...
	for (i = 1; i < S_NMOONS; i++) {
		...
		md[i].ra = (float)(satra + dra);
		md[i].dec = (float)(satdec - ddec);
	}
}
