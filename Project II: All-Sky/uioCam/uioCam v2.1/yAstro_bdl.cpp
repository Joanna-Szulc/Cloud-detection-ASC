...
#include <string.h>
#include <math.h>
/* #include <unistd.h>  NOT in Visual C++ */
...
read_bdl(FILE *fp, double jd, double *xp, double *yp, double *zp, char ynot[])
{
	...
	for (i = 0; i < nsat; i++) {
		...
		if (fseek(fp, os, SEEK_SET) < 0) {
			sprintf_s(ynot, 36, "Seek error to %ld for rec %d", os, id);
			return (-1);
		}
...
readField(FILE *fp, ScanType f, int width, void *ptr, char ynot[])
{
	...
	if (width > sizeof(buf) - 1) {
		sprintf_s(ynot, 64, "BDL Field width %d > %d", width, (int)sizeof(buf));
		return (-1);
	}
	if (width != (int)fread(buf, 1, width, fp)) {
		if (ferror(fp)) strcpy_s(ynot, 64, "BDL IO error");
		else if (feof(fp)) strcpy_s(ynot, 64, "BDL unexpected EOF");
		else strcpy_s(ynot, 64, "BDL short file");
		return (-1);
	}
	...
	case NL:
		...
		break;
	default:
		sprintf_s(ynot, 64, "Bug! format = %d", f);
		return (-1);
	}
	return (0);
}
