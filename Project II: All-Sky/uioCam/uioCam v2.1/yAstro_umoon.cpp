...
#define URAU    .0001597        /* Uranus radius, AU */
	...
	for (i = 1; i < U_NMOONS; i++) {
		md[i].x = (float)(x[i - 1] / URAU);	/* we want u radii +E */
		md[i].y = (float)(-y[i - 1] / URAU);	/* we want u radii +S */
		md[i].z = (float)(-z[i - 1] / URAU);	/* we want u radii +front */
	}
	...
	MoonData md[U_NMOONS])	/* fill in RA and Dec */
{
	...
	for (i = 1; i < U_NMOONS; i++) {
		...
		md[i].ra = (float)(ura + dra);
		md[i].dec = (float)(udec - ddec);
	}
}
