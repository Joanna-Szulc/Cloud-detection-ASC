...
#define	JUPRAU	.0004769108	/* jupiter radius, AU */
	...
	/* open */
	(void)sprintf_s(buf, 1024, "%s/%s", dir, fn);
	errNo = fopen_s(&fp, buf, "r");
	if (!fp) {
	...
	for (i = 1; i < J_NMOONS; i++) {
		md[i].x = (float)(x[i - 1] / JUPRAU);	/* we want jup radii +E */
		md[i].y = (float)(-y[i - 1] / JUPRAU);	/* we want jup radii +S */
		md[i].z = (float)(-z[i - 1] / JUPRAU);	/* we want jup radii +front */
	}
...
#define	dcos(x)	cos(degrad(x))
	...
	*cmlII = degrad(290.28 + 870.1869088*(d - Del / 173) + psi - B);
	...
	md[1].x = (float)(-r1 * dsin(u1 + cor_u1));
	md[2].x = (float)(-r2 * dsin(u2 + cor_u2));
	md[3].x = (float)(-r3 * dsin(u3 + cor_u3));
	md[4].x = (float)(-r4 * dsin(u4 + cor_u4));
	...
	md[1].y = (float)(z1*dsinDe);
	md[2].y = (float)(z2*dsinDe);
	md[3].y = (float)(z3*dsinDe);
	md[4].y = (float)(z4*dsinDe);
	...
	MoonData md[J_NMOONS])	/* fill in RA and Dec */
{
	...
	for (i = 1; i < J_NMOONS; i++) {
		...
		md[i].ra = (float)(jupra + dra);
		md[i].dec = (float)(jupdec - ddec);
	}
}
