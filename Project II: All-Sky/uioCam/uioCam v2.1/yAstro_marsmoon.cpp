	...
	MoonData md[M_NMOONS])		/* return info */
{
	...
	*sizep = degrad(mop->s_size / 3600.0);
	...
	md[1].mag = (float)(11.8 + dmag);
	md[2].mag = (float)(12.9 + dmag);
...
#define MRAU    .00002269       /* Mars radius, AU */
	...
	for (i = 1; i < M_NMOONS; i++) {
		md[i].x = (float)(x[i - 1] / MRAU);	/* we want mars radii +E */
		md[i].y = (float)(-y[i - 1] / MRAU);	/* we want mars radii +S */
		md[i].z = (float)(-z[i - 1] / MRAU);	/* we want mars radii +front */
	}
	...
	MoonData md[M_NMOONS])	/* fill in RA and Dec */
{
	...
	for (i = 1; i < M_NMOONS; i++) {
		...
		md[i].ra = (float)(mra + dra);
		md[i].dec = (float)(mdec - ddec);
	}
}
