...
{
	strcpy_s(builtin[idx].o_name, 21, name);
	builtin[idx].o_type = PLANET;
	builtin[idx].pl_code = (PLCode)pl;
	builtin[idx].pl_moon = (MCode)moon;
}
...
setMoon(Now *np, Obj *moonop, Obj *planop, MoonData *mdp)
{
	...
	rotate(pa, &dra, &ddec);
	moonop->s_alt = (float)(planop->s_alt + ddec);
	moonop->s_az = (float)(planop->s_az - dra / cos(planop->s_alt));

	/* new mag directly from mdp */
	set_smag(moonop, mdp->mag);

	/* name */
	strcpy_s(moonop->o_name, 21, mdp->full);
}
