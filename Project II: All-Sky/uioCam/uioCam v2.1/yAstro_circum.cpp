...
obj_cir(Now *np, Obj *op)
{
	...
	case FIXED2:	 return (obj_fixed(np, op));
...
obj_2binary(Now *np, Obj *op)
{
	if (op->b_nbp > 0) {
		...
		bp->bo_sep = (float)rho;
		rho = (float)degrad(rho / 3600.);	/* arc secs to rads */
		bp->bo_dec = (float)op->s_dec + (float)(rho*cos(theta));
		bp->bo_ra = (float)(op->s_ra + rho * sin(theta) / cos(op->s_dec));
	}
